...
void AliAnalysisTaskSexaquark::ProcessFindableSexaquarks() {
    ...
    for (UInt_t reactionIdx = 600; reactionIdx < 620; reactionIdx++) {
        ...
        for (Int_t& esdIdx : getEsdIdx_fromReactionIdx[reactionIdx]) {
            AliInfoF("reacID, esdIdx, mcIdx, pdgCode = %i, %i, %i, %i", reactionIdx, esdIdx, getMcIdx_fromEsdIdx[esdIdx],
                     getPdgCode_fromMcIdx[getMcIdx_fromEsdIdx[esdIdx]]);
        ...
        AliInfoF("-> it worked with reacID = %i !!!", reactionIdx);
...
void AliAnalysisTaskSexaquark::KalmanSexaquarkFinder_ChannelD() {
    ...
    for (Int_t idxAntiLambda = 0; idxAntiLambda < (Int_t)kfAntiLambdas.size(); idxAntiLambda++) {
        for (Int_t& esdIdxPosKaon : esdIndicesOfPosKaonTracks) {
            ...
            chi2ndf = (Double_t)kfAntiSexaquark.GetChi2() / (Double_t)kfAntiSexaquark.GetNDF();  // exclusive to KF
            ...
            fHist_AntiSexaquarks[std::make_tuple("Found", "All", "Chi2ndf")]->Fill(chi2ndf);
            ...
            fHist_AntiSexaquarks[std::make_tuple("Found", "Signal", "Chi2ndf")]->Fill(chi2ndf);
    ...
    for (Int_t idxAntiLambda = 0; idxAntiLambda < (Int_t)kfAntiLambdas.size(); idxAntiLambda++) {
        for (Int_t idxPionPair = 0; idxPionPair < (Int_t)kfPionPairs.size(); idxPionPair++) {
            for (Int_t& esdIdxPosKaon : esdIndicesOfPosKaonTracks) {
                ...
                chi2ndf = (Double_t)kfAntiSexaquark.GetChi2() / (Double_t)kfAntiSexaquark.GetNDF();  // exclusive to KF method
                ...
                fHist_AntiSexaquarks[std::make_tuple("Found", "All", "Chi2")]->Fill(kfAntiSexaquark.GetChi2());
                fHist_AntiSexaquarks[std::make_tuple("Found", "All", "Chi2ndf")]->Fill(chi2ndf);
                ...
                fHist_AntiSexaquarks[std::make_tuple("Found", "Signal", "Chi2")]->Fill(kfAntiSexaquark.GetChi2());
                fHist_AntiSexaquarks[std::make_tuple("Found", "Signal", "Chi2ndf")]->Fill(chi2ndf);
