 ...
 *
 */
void QA_ChannelE(TString input_dir = "output", TString output_dir = "gfx") {

    std::vector<TString> InputFilenames = {"ChannelE1.73.root", "ChannelE1.8.root", "ChannelE1.87.root", "ChannelE1.94.root", "ChannelE2.01.root"};
    TString ListName = "V0s_Hists";

    /* Case 1: V0s */

    std::vector<TString> V0sName = {"AntiLambda", "PionPair"};
    ...
    for (Int_t nn = 0; nn < (Int_t)V0sName.size(); nn++) {
        ...
        c1->Print(output_dir + "/ChannelE_" + V0sName[nn] + "_Mass.png");
    ...
    system("mkdir -p " + output_dir);
    c2->Print(output_dir + "/ChannelE_" + HistName + ".png");
